...
// banner图片ul
        var bannerList = $id('banner-list');
        var bannerli = $tag(bannerList, 'li');
// banner切换按钮ul
        var btnList = $id('banner-btn');
        var bannerBtnli = $tag(btnList, 'li');
// banner前后按钮ul
        var pnbtng = $id('pnbtn-g');
        var pnbtn = $tag(pnbtng, 'li');
// banner外部盒子
        var bannerWrap = $id('banner-wrapper');
// 视口宽度
        var clientW;
// 记忆顺序
        var jiyi = 2;
...
// banner轮播运动函数
        function banner(num) {
            ...
            var divH = 547 / numArr[num];
            // 记忆顺序 背景图
            ots(jiyi);
            divsum.style.background = 'url(images/banner' + jiyi + '.jpg) no-repeat center';
            ...
            for (var j = 0; j < numArr[num]; j++) {
                for (var i = 0; i < numArr[num]; i++) {
                    ...
                    bgdiv.style.top = divH * j + 'px';
                    bgdiv.style.background = 'url(images/banner' + num + '.jpg) -' + (divw * i + bannerleft) + 'px ' + (-(divH * j)) + 'px';
            ...
            // 替换背景图
            function bannerbg() {
                bannerli[num].style.background = 'url(images/banner' + num + '.jpg) no-repeat center';
...
// 分类轮播按钮
        ...
        var flPbtn = $tag(flBtnG, 'li')[0];
        var flNbtn = $tag(flBtnG, 'li')[1];
        var flList = $id('fenlei-list');
        var flwFlag = 1;
        var aboutFlag = 1;
        var fybtnflag = 1;
        var about = $id('about');
            ...
            if (clientW > 784 && index && fyflag < 7) {
                ...
                function fyani() {
                    ...
                    if (index == 1) {
                        ...
                        if (sTop >= 670 * i - 6 || sTop <= 1) {
                            window.scrollTo(0, 670 * i);
                            navliHl();
                        ...
                        if (sTop <= 670 * (fyflag - 1) + 5 || sTop < 6) {
                            window.scrollTo(0, 670 * (fyflag - 1))
                            navliHl();
                            clearInterval(fyTimeId);
                            gdflag = 0;
                        }
...
// 滚动监听
        window.onscroll = function (e) {
            ...
            if (newsFlag) {
                // 获得滚动条卷去的距离
                sTop = document.documentElement.scrollTop || document.body.scrollTop;
                news.style = 'background-position:center ' + (sTop - 1580) / 4 + 'px';
            }
